...
class Node:
    ...
    def __init__(self,up=None,down=None,col=None,left=None,right=None,row=None):
        ...
        self.row    = row       # the number of this row
        self.left    = left       # reference to the previous 1 in this row
        self.right   = right     # reference to the next 1 in this row
...
class Dancer:
    ...
    def readRows(self, headers, matrix):
        ...
        for num, row in enumerate(matrix):
            ...
            for col in row[:-1]:           # for each column with a 1 in this row
                ...
                n = Node(up = h.up, down = h, col = h, row = num, left = curNode)
                h.length += 1    
             ...
             # finish off the row by hookig the last node to the first   
    ...
    def cover(self, col):
        ...
        while rr.row != 'head':     # for each one in column
            nn = rr.right                        
            while nn.col != col:     # for each other node in this row
                uu = nn.up
                dd = nn.down
                uu.down = dd
                dd.up   = uu               # remove nn from its column
                nn.col.length -= 1
                nn = nn.right
                updates += 1
            rr = rr.down      # next row
    ...
    def uncover(self, col):
        ...
        while rr.row != 'head':
            nn= rr.left                                 # previous column in row rr
            while nn.col != col:
                uu = nn.up
                dd = nn.down
                uu.down = dd.up = nn
